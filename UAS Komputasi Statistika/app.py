...
st.image("alur_flowchart.jpg", caption="Flowchart Simulasi Data Sleep & Lifestyle Study", use_column_width=True)
...
st.markdown("Missing Values:") # Menampilkan jumlah missing values untuk setiap kolom dalam dataset
st.markdown(df.isnull().sum())

st.markdown("Outliers:") # Menampilkan jumlah outliers untuk setiap kolom numerik dalam dataset menggunakan metode IQR (Interquartile Range)
for column in df.select_dtypes(include=[np.number]).columns:
    Q1 = df[column].quantile(0.25)
    Q3 = df[column].quantile(0.75)
    IQR = Q3 - Q1
    outliers = df[(df[column] < (Q1 - 1.5 * IQR)) | (df[column] > (Q3 + 1.5 * IQR))]
    st.markdown(f"{column}: {len(outliers)} outliers")

st.markdown("Descriptive Statistics:") # Menampilkan statistik deskriptif untuk setiap kolom numerik dalam dataset
...
st.markdown("Scatterplot antara Durasi Tidur dan Kualitas Tidur:")
st.pyplot(fig7)
...
st.markdown("Feature engineering adalah proses menciptakan fitur baru dari data yang ada untuk meningkatkan kinerja model. " \
"Dalam konteks ini, saya melakukan feature engineering dengan membuat kategori untuk durasi tidur berdasarkan rentang waktu yang telah ditentukan, seperti sangat kurang, kurang, normal, cukup, dan kesiangan. " \
"Kategori ini akan membantu kita memahami bagaimana durasi tidur dapat mempengaruhi kualitas tidur, dan memberikan wawasan tentang bagaimana individu dapat dikategorikan berdasarkan durasi tidur mereka untuk analisis lebih lanjut.")

# Buat Katedori Tidur dari Variabel Durasi Tidur

df["Sleep Category"] = pd.cut(df["Sleep Duration"], bins=[0, 4, 7, 9, 12, np.inf], labels=["Sangat Kurang", "Kurang", "Normal", "Cukup", "Kesiangan"]) # Membuat kategori untuk durasi tidur berdasarkan rentang waktu yang telah ditentukan
st.markdown("Kategori Durasi Tidur")
st.markdown(df.head(10))
st.markdown("Distribusi Kategori Durasi Tidur:")
st.markdown(df["Sleep Category"].value_counts())
...
"Feature engineering ini membantu kita memahami bagaimana individu dapat dikategorikan berdasarkan durasi tidur mereka, dan memberikan dasar yang kuat untuk analisis lebih lanjut.")
...
st.markdown("Simulasi Monte Carlo adalah metode statistik yang digunakan untuk memodelkan probabilitas dari berbagai hasil dalam suatu proses yang tidak dapat diprediksi secara pasti. " \
...
st.markdown("Visualisasi Simulasi Monte Carlo untuk Durasi Tidur, Tingkat Aktivitas Fisik, dan Tingkat Stres:")
st.pyplot(fig8)
...
st.markdown("Markov Chain adalah model matematika yang digunakan untuk memodelkan sistem yang berubah dari satu state ke state lainnya, di mana probabilitas transisi antara states tersebut hanya bergantung pada state saat ini. " \
"Dalam konteks ini, saya menggunakan Markov Chain untuk memodelkan transisi antara berbagai tingkat kualitas tidur dalam dataset Sleep & Lifestyle Study. " \
"Model ini akan membantu kita memahami pola transisi antara berbagai tingkat kualitas tidur dalam dataset, dan bagaimana faktor-faktor seperti durasi tidur, aktivitas fisik, dan tingkat stres dapat mempengaruhi transisi antara berbagai tingkat kualitas tidur dalam dataset, serta memberikan wawasan tentang bagaimana kualitas tidur dapat dipengaruhi oleh faktor-faktor tersebut berdasarkan pola transisinya.")

# Siapkan Data untuk Markov Chain
states = df["Quality of Sleep"].unique() # Mengambil nilai unik dari kolom "Quality of Sleep" untuk digunakan sebagai states dalam Markov Chain
transition_matrix = pd.crosstab(states.shift(), states, normalize="index") # Membuat matriks transisi dengan menggunakan crosstab untuk menghitung frekuensi transisi antara states dan menormalkannya untuk mendapatkan probabilitas transisi

st.markdown("Matriks Transisi Markov Chain:")
st.markdown(transition_matrix)

# Visualisasi Data Markov Chain
fig9, ax9 = plt.subplots() # Membuat heatmap dari matriks transisi Markov Chain 
...
ax9.set_xlabel("State Saat Ini")
ax9.set_ylabel("State Berikutnya")
st.markdown("Visualisasi Matriks Transisi Markov Chain untuk Kualitas Tidur:")
st.pyplot(fig9)

st.markdown("Berdasarkan hasil dari Markov Chain, kita dapat melihat pola transisi antara berbagai tingkat kualitas tidur dalam dataset Sleep & Lifestyle Study. " \
"Model ini memberikan wawasan tentang bagaimana faktor-faktor seperti durasi tidur, aktivitas fisik, dan tingkat stres dapat mempengaruhi transisi antara berbagai tingkat kualitas tidur dalam dataset, serta memberikan wawasan tentang bagaimana kualitas tidur dapat dipengaruhi oleh faktor-faktor tersebut berdasarkan pola transisinya. " \
...
st.markdown(df[["Sleep Duration", "Physical Activity Level", "Stress Level", "Hidden State"]].head(10)) # Menampilkan 10 baris pertama dari dataframe yang berisi kolom "Sleep Duration", "Physical Activity Level", "Stress Level", dan "Hidden State"
...
ax10.legend()
st.markdown("Visualisasi Pola Tersembunyi dalam Data Durasi Tidur berdasarkan Hidden Markov Model (HMM):")
st.pyplot(fig10)  
