...
st.dataframe(df[["Sleep Duration", "Sleep Category"]].head(10)) # Menampilkan 10 baris pertama dari dataframe yang berisi kolom "Sleep Duration" dan "Sleep Category"
...
st.markdown("Markov Chain adalah model matematika yang digunakan untuk memodelkan sistem yang berubah dari satu state ke state lainnya, di mana probabilitas transisi hanya bergantung pada state saat ini. " \
...
"Model ini akan membantu kita memahami pola transisi antara berbagai tingkat kualitas tidur dalam dataset, serta memberikan wawasan tentang bagaimana faktor-faktor seperti durasi tidur, aktivitas fisik, dan tingkat stres dapat mempengaruhi transisi antara berbagai tingkat kualitas tidur dalam dataset ini.")

# Siapkan Data untuk Markov Chain
states = df["Quality of Sleep"].unique() # Mendapatkan unique states dari variabel Kualitas Tidur dalam dataset
transition_matrix = pd.DataFrame(0, index=states, columns=states) # Membuat matriks transisi dengan nilai awal 0
for i in range(len(df) - 1):
    current_state = df["Quality of Sleep"].iloc[i] # Mendapatkan state saat ini dari variabel Kualitas Tidur
    next_state = df["Quality of Sleep"].iloc[i + 1] # Mendapatkan state berikutnya dari variabel Kualitas Tidur
    transition_matrix.loc[current_state, next_state] += 1 # Menambahkan 1 ke matriks transisi untuk transisi dari state saat ini ke state berikutnya
transition_matrix = transition_matrix.div(transition_matrix.sum(axis=1), axis=0) # Normalisasi matriks transisi dengan membagi setiap baris dengan jumlah total transisi dari state tersebut 
...
"Model ini memberikan wawasan tentang bagaimana faktor-faktor seperti durasi tidur, aktivitas fisik, dan tingkat stres dapat mempengaruhi transisi antara berbagai tingkat kualitas tidur dalam dataset ini. " \
"Informasi ini sangat berguna untuk mengembangkan strategi untuk meningkatkan kualitas tidur berdasarkan faktor-faktor yang terjadi dalam analisis ini.")
